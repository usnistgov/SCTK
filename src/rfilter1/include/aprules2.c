...
/* code: */
  ...
  *perr = 0;
  ...
  if (rset == NULL)
    {pb = (Char *)strcpy((char *)pb,(char *)pa);
     goto RETURN;
    }
...
if (db_level > 1) printf("%s jrule=%d, irule=%d\n",pdb,jrule,irule);
 if (streq((char *)rset->format,"NIST1"))
          {if ( ((rset->case_sensitive)&&
                 (strncmp((char *)pi,(char *)(rset->rule[irule].sin),rset->rule[irule].sinl)==0)&&
                 (strncmp((char *)(pi-rset->rule[irule].lcontextl),
                          (char *)rset->rule[irule].lcontext,
                          rset->rule[irule].lcontextl)==0)&&
                 (strncmp((char *)(pi+rset->rule[irule].sinl),
                          (char *)rset->rule[irule].rcontext,
                          rset->rule[irule].rcontextl)==0))
             || ((!rset->case_sensitive)&&
                 (strncasecmp((char *)pi,(char *)rset->rule[irule].sin,rset->rule[irule].sinl)==0)&&
                 (strncasecmp((char *)(pi-rset->rule[irule].lcontextl),
			      (char *)rset->rule[irule].lcontext,
                          rset->rule[irule].lcontextl)==0)&&
                 (strncasecmp((char *)(pi+rset->rule[irule].sinl),
			      (char *)rset->rule[irule].rcontext,
                          rset->rule[irule].rcontextl)==0))
	      )
             {hit = T;
	       pb = (Char *)strcat((char *)pb,(char *)rset->rule[irule].sout);
...
pdb,irule,rset->rule[irule].lcontextl,
			 bool_print((strncasecmp((char *)(pi-rset->rule[irule].lcontextl),
						 (char *)(rset->rule[irule].lcontext),
                    rset->rule[irule].lcontextl)==0)));
          }
       }
